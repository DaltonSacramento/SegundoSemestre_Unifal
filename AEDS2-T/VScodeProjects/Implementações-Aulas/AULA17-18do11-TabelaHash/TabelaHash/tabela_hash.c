...
int insere_hash_enc_separado(hash * ha, estudante * e) {
	...
	if (ha->estudantes[pos] != NULL) {
		lista *noanovo=malloc(sizeof(lista));
		lista *aux;
		aux->no=ha->estudantes[pos];
		while(aux->prox!=NULL){
			aux=aux->prox;
		}
		noanovo->no=e;
		...
		lista *noa=malloc(sizeof(lista));
		noa->no=e;
		noa->prox=NULL;
		ha->estudantes[pos] = noa->no;
		return 1;
	}
	return 0;
}
...
char* busca_hash_enc_separado(hash * ha, int matricula) {
	...
	for (i = 0; i < ha->TABLE_SIZE; i++) {
		...
		if (ha->estudantes[pos] == NULL) {
			...
			aux->no=ha->estudantes[pos];
			while(aux->prox!=NULL){
				if(aux->no->matricula==matricula){
					return &(aux->no->nome[0]);
				}
				aux=aux->prox;
			}
		}
